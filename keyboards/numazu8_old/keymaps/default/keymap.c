...
#define KEY_THRESHOLD 12 /* bigger than 1.2 */
...
uprintf(" \n");
    ...
    uprintf("%u  \n",(uint32_t)get_key_val(3, 1));
      ...
      //uprintf("%u \n ",(uint32_t)get_key_val(3, 0)*10);
       //uprintf("%u \n ",((uint32_t)calibrate_result[3][0])*KEY_THRESHOLD);

  for(uint8_t col=0;col<CAPMATRIXCOL;col++){
    for(uint8_t row=0;row<1;row++){
        //uprintf("get_key_val col[%d]  %u \n ", col,(uint32_t)get_key_val(col, row));
...
//          uprintf("calibrate_result %u \n ", ((uint32_t)calibrate_result[col][row]));
          //uprintf("get_key_val col[%d] row[%d] %u \n ", col,row,(uint32_t)get_key_val(col, row));
